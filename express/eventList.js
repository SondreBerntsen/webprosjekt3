...
eventList.get("/", (req, res) => {
    ...
    if (year) {
        SELECT_ALL_EVENTS_QUERY = `SELECT * FROM events WHERE year(date)='${year}'`;
    } else {
        SELECT_ALL_EVENTS_QUERY = `SELECT * FROM events WHERE year(date)=(SELECT MAX(year(date)) FROM events)`;
    }
